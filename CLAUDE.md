...
Measuring how well a model reads real documents (needs `eval-docs/` and a key; nothing is committed):

```powershell
$env:ANTHROPIC_API_KEY = "..."
npm run eval:extraction                      # read and score every document in eval-docs/
npm run eval:extraction -- --record          # keep the answers for replay
npm run eval:extraction -- --replay          # score the kept answers again, free
npm run eval:extraction -- --model claude-sonnet-5
```

## Status

- Branch for Phase 3 onward: `claude/travelos-phase-3-41f57b` (fast-forward of `claude/travelos-operating-system-581b62`, which holds Phases 0–2 up to `25d07f7b`). Phase 6 continues on `claude/admiring-edison-ibfdfg`, a fast-forward of it. Nothing merges to `main`.
...
- **Phase 5 — document intelligence ☑** (5.0 AI provider interface + honest status · 5.1 document centre · 5.2 reading documents into proposals · 5.3 evaluation harness). Module detail in `docs/travelos/PROGRESS.md`. The owner tested Phase 4 and said continue on 2026-09-21; **Phase 5 is at its checkpoint, waiting for the owner's manual test** — reading needs `ANTHROPIC_API_KEY`, and the harness needs their own PDFs in `eval-docs/`.
- **Phase 6 — copilot and insights ☑ (at its checkpoint)** on `claude/admiring-edison-ibfdfg` (fast-forward of the Phase 3 branch): 6.0 the ask loop with per-call permission re-check and `ai_actions` · 6.1 **Ask TravelOS** screen · 6.2 proposals a person approves (task, follow-up, message draft, trip change) · 6.3 **Needs attention** insights, counted by queries. Detail in `docs/travelos/PROGRESS.md`; waiting for the owner's manual test. A live copilot needs `ANTHROPIC_API_KEY`.
- **Phase 7 — communication and automation ☑** (7.0 message templates · 7.1 channels with honest status (Meta WhatsApp Cloud API, SMTP) · 7.2 one communications log per record, sending on the job runner, signed delivery webhook · 7.3 notifications · 7.4 automation rules with run history, classic scheduler retired). Detail in `docs/travelos/PROGRESS.md`.
- **Phase 8 — customer portal ☑** (private link + optional one-time code, strict read models, per-family scoping, feedback). Portal code: `modules/portal/*`, public routes `server/src/routes/portal.ts`, page `src/portal/`. Set `PUBLIC_APP_URL` so links can be sent in messages.
- **Phase 9 — analytics and platform ☑** (9.0 Reports · 9.1 custom roles as principals `custom:<id>` · 9.2 read-only API keys `apikey:<id>` · 9.3 organisation settings, setup checklist, platform onboarding · 9.4 supplier scorecards; the shared network is designed, not built). Detail in `docs/travelos/PROGRESS.md`.
- **Phases 7, 8 and 9 are waiting for the owner's manual test** (the owner said "continue all" after Phase 6 on 2026-09-26). The owner tests manually at the end of each checkpoint.
- Platform facts worth knowing: `hasPermission(role, …)` also accepts principal keys (`custom:<id>`, `apikey:<id>`) whose permissions `requireAuth` loads via `core/principals.ts` — keep every new check going through it. Never run two integration suites at once: they share `travelos_test`.
- Comms facts worth knowing: sends go through `modules/comms/service.ts` (never an adapter directly); tests use `COMMS_TRANSPORT=memory` and `captured` from `server/src/comms`; automation rules live in `src/shared/calc/automation.ts` + `modules/automation`.
- Copilot facts worth knowing: tools live in `modules/copilot/tools.ts` (reads) and `writeTools.ts` (proposals with `apply`); each declares its permission. Recorded chat replays are keyed on the conversation so far (`chatKey`, key-sorted JSON), so tests script an exchange by running the same tools — see `script()` in `tests/integration/copilot.v2.test.ts`.
