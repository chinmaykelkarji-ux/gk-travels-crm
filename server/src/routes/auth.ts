...
router.post('/dev-login', async (_req, res) => {
  // DEVELOPMENT BYPASS — only active in NODE_ENV=development
  // REMOVE BEFORE PRODUCTION
  ...
  try {
    const user = await prisma.user.findUnique({
      where: { email: 'admin@gktravels.local' },
    });

    if (!user || !user.isActive) {
      res.status(500).json({ error: 'Dev admin user not found — run create-admin script' });
      return;
    }
