...
| P9-1 | The four system roles stay in code; custom roles are rows with a base role and an explicit permission list, and a user with one acts as the principal `custom:<id>`. `requireAuth` loads its permissions (cached a minute, cleared at once on change) so the single `hasPermission()` used everywhere honours it; an unknown principal has no permissions. A custom role can never grant "everything", the driver view or managing people; the owner and drivers keep their roles. | One check, no parallel system: routes, copilot tools, redaction and insights all go through `hasPermission()` already, so custom roles work everywhere without touching each call site — and cannot be forgotten in a new one. Managing people stays with the owner so nobody can raise their own access. |
| P9-2 | API keys are read-only by construction (":read" permissions only, GET only, `/api/v2` only), hashed, shown once, and act through the same permission check and redaction as staff. | An integration that can write is a second, less-watched door into the books. Reading is what exports and dashboards need. |
| P9-3 | Organisation settings are read through `readSettings()`, which keeps each valid value and defaults the rest; new organisations are created only by the platform operator (`PLATFORM_ADMIN_SECRET`), with everything else written on first use. | A bad value must never break a screen. Onboarding a second agency is an operator's act, not something any signed-in owner can do. |
| P9-4 | Supplier network: only scorecards from the organisation's own records are built. Sharing ratings across organisations (a shared supplier directory keyed by GSTIN / phone, opt-in, aggregated so no organisation's prices or volumes are exposed) is designed in architecture H.9 and not built. | Sharing supplier data between agencies needs the owner's decision on what may be shared and consent from the other side; building it before that would be guessing. |
| P9-5 | Deferred: removing the `organizationId` column default (every nested write would have to set it, and production needs a coordinated cutover), and moving the classic screens' `RoleGuard` from roles to permissions (custom roles see classic screens by their base role until those screens are replaced). | Both touch legacy code paths that still work; rule 12 says keep them working until the new module replaces them. |
